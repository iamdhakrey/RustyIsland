...
typeof DYNAMIC_ISLAND_STATES;

const PLAYERCTL_METADATA_CMD = `playerctl metadata --format '{\
                                    "artist":"{{artist}}",\
                                    "album":"{{album}}",\
                                    "title":"{{title}}",\
                                    "status":"{{status}}",\
...
export default class DynIslandExtension extends Extension {
    ...
    private musicMetadata: any | null;
    ...
    disable() {
        if (this.dynamicIsland) {
            (Main.panel as any)._centerBox.remove_child(this.dynamicIsland);
    ...
    _createDynamicIsland() {
        ...
        this.dynamicIsland.connect('enter-event', () => {
            if (this.dynamicIsland) {
                this.dynamicIsland.add_style_pseudo_class('hover');
            }
        });

        this.dynamicIsland.connect('leave-event', () => {
            if (this.dynamicIsland) {
                this.dynamicIsland.remove_style_pseudo_class('hover');
            }
        });
    }
    ...
    _createCompactContent() {
        ...
        if (this.islandContainer) {
            this.islandContainer.add_child(this.compactContent);
        }
    ...
    _createExpandedContent() {
        ...
        if (this.islandContainer) {
            this.islandContainer.add_child(this.expandedContent);
        }
    }
    ...
    _setupPlayerctl() {
        ...
        try {
            let [success, stdout, stderr] = GLib.spawn_command_line_sync('which playerctl');
            if (!success) {
    ...
    _updateMusicFromBackend() {
        try {
            ...
            let [status, statusOut] = GLib.spawn_command_line_sync(PLAYERCTL_METADATA_CMD)
            if (!status) {
            ...
            let musicFile = new TextDecoder().decode(statusOut).trim();
            let musicData = JSON.parse(musicFile);
    ...
    _updateMusicDisplay(musicData: MusicMetadata) {
        if (musicData.status === 'playing' || musicData.title !== 'No Music Playing') {
            // Update track info
            let title = (musicData.title || 'Unknown Track').substring(0, 20);
            let artist = (musicData.artist || 'Unknown Artist').substring(0, 20);
            ...
            if (musicData.artUrl) {
                if (this.albumArt) {
                    this.albumArt.gicon = Gio.icon_new_for_string(musicData.artUrl);
                }
            }
            ...
            if (this.playPauseButton && this.playPauseButton.child) {
                ...
                icon.icon_name = musicData.status === 'playing' ?
            ...
            log(`Dynamic Island: Music - ${title} by ${artist} (${musicData.status === 'playing' ? 'Playing' : 'Paused'})`);
        } else {
            // No music playing
            if (this.trackTitle && this.trackArtist) {
    ...
    _showCompactContent() {
        if (this.compactContent) {
            this.compactContent.visible = true;
        }
        if (this.expandedContent) {
            this.expandedContent.visible = false;
        }
        if (this.dynamicIsland) {
    ...
    _showExpandedContent() {
        if (this.compactContent) {
            this.compactContent.visible = false;
        }
        if (this.expandedContent) {
            this.expandedContent.visible = true;
        }
        if (this.dynamicIsland) {
        ...
        switch (state) {
            case DYNAMIC_ISLAND_STATES.NOTIFICATION:
                if (this.musicSection) {
                    this.musicSection.visible = false;
                }
            ...
            case DYNAMIC_ISLAND_STATES.MUSIC:
                ...
                if (this.musicSection) {
                    this.musicSection.visible = true;
                }
                this._showExpandedContent();
                break;
    ...
    _addToPanel() {
        ...
        if (this.dynamicIsland) {
            (Main.panel as any)._centerBox.insert_child_at_index(this.dynamicIsland, 0);
